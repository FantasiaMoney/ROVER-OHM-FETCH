...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    constructor(address _factoryAddress, address _WETHAddress) public {
    ...
    function addLiquidityETH(
        ...
        address pair = UniswapV2Library.pairFor(factoryAddress, token, WETHAddress);
    ...
    function swapTokensForExactTokens(
        ...
        amounts = UniswapV2Library.getAmountsIn(factoryAddress, amountOut, path);
    ...
    {
        require(path[0] == WETHAddress, 'UniswapV2Router: INVALID_PATH');
        amounts = UniswapV2Library.getAmountsOut(factoryAddress, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETHAddress).deposit{value: amounts[0]}();
        assert(IWETH(WETHAddress).transfer(UniswapV2Library.pairFor(factoryAddress, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETHAddress, 'UniswapV2Router: INVALID_PATH');
        amounts = UniswapV2Library.getAmountsIn(factoryAddress, amountOut, path);
        require(amounts[0] <= amountInMax, 'UniswapV2Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, UniswapV2Library.pairFor(factoryAddress, path[0], path[1]), amounts[0]
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = UniswapV2Library.sortTokens(input, output);
            IUniswapV2Pair pair = IUniswapV2Pair(UniswapV2Library.pairFor(factoryAddress, input, output));
            uint amountInput;
            uint amountOutput;
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, UniswapV2Library.pairFor(factoryAddress, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETHAddress).transfer(UniswapV2Library.pairFor(factoryAddress, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20Uniswap(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20Uniswap(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        uint amountOut = IERC20Uniswap(WETHAddress).balanceOf(address(this));
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return UniswapV2Library.getAmountsOut(factoryAddress, amountIn, path);
    }
    ...
    function getAmountsIn(uint amountOut, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return UniswapV2Library.getAmountsIn(factoryAddress, amountOut, path);
    }
}
